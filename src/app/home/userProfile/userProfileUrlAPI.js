...
export async function updateUserProfileImage(userId, base64Data) {
    try {
        ...
        const getUserProfileResponse = await fetch(`/api/userprofiles`);
        ...
        if (existingUserData) {
            ...
            const putResponse = await fetch(`/api/userprofiles/${userId}`, {
                method: "PUT",
                body: JSON.stringify({
            ...
            const postResponse = await fetch(`/api/userprofiles`, {
                method: "POST",
                body: JSON.stringify({
                    UserId: userId,
                    ProfileUrl: base64Data,
